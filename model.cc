...
TrPQPModel::TrPQPModel(std::string filePath) : filePath(filePath)
{
    ...
    std::unique_ptr<PQP_Model> obj1 = std::make_unique<PQP_Model>();
    ...
    this->pqpModel = std::move(obj1);

    this->R = Matrix<PQP_REAL, 3, 3>::Identity(); // Initialized as identity matrix
    this->t = Vector3<PQP_REAL>::Zero();          // Initialized as zero vector
}

TrPQPModel::TrPQPModel(std::unique_ptr<PQP_Model> model, std::string filePath)
...
void TrPQPModel::CheckDistance(PQP_DistanceResult *result, PQP_REAL rel_err, PQP_REAL abs_err, TrPQPModel *m1, TrPQPModel *m2)
{
    PQP_Distance(result, m1->getR(), m1->getT(), m1->pqpModel.get(), m2->getR(), m2->getT(), m2->pqpModel.get(), rel_err, abs_err);
}

void TrPQPModel::Collide(PQP_CollideResult *result, TrPQPModel *m1, TrPQPModel *m2)
{
